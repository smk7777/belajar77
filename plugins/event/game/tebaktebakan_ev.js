   ...
   async: async (m, {
      ...
      try {
         ...
         if (m.quoted && /apatuh/i.test(m.quoted.text)) {
            if (!(id in client.tebaktebakan) && /apatuh/i.test(m.quoted.text)) return client.reply(m.chat, Func.texted('bold', `Soal tersebut telah berakhir, silahkan kirim _${prefixes[0]}tebaktebakan_ untuk mendapatkan soal baru.`), m)   
            if (m.quoted.id == client.tebaktebakan[id][0].id) {
               ...
               if (body.toLowerCase() == json.jawaban.toLowerCase()) {
                  await client.sendSticker(m.chat, await Func.fetchBuffer('./media/image/true.webp'), m, {
                     ...
                     users.uang += reward
                     clearTimeout(client.tebaktebakan[id][2])
